...
class Board extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            squares: [],
            marioLocation: Math.floor(Math.random() * (this.totalSquares - 0)),
    ...
    componentDidUpdate() {
        setTimeout(() => {
            this.checkForSprites();
        }, 100);
    }

    handleClick(i) {
        if (i === this.state.marioLocation) this.checkForSprites();
    }
    ...
    decideMove(spriteLocation) {
        console.log('Sprites Locations: ', spriteLocation);
        let distance = Math.abs(
            spriteLocation[0] - this.state.marioLocation
        );
        let marioRange = this.getMarioRange();
        if (
            distance < this.props.width
            && spriteLocation[0] < this.state.marioLocation
            && this.numberInRange(spriteLocation[0], marioRange)
        ) {
            this.moveX('-');
        } else if (
            distance < this.props.width
    ...
    checkForSprites() {
        console.log('checking for sprites: ', this.state);
        let sprites = this.state.squares.filter(square => {
            return square.element === 'enemy.png';
        });
        console.log('Total sprites: ', sprites);
        if (sprites.length === 0) {
            alert('Game over. Total moves to hit all the green sprites: ' + this.state.moves);
        } else {
            this.decideMove(sprites.map((sprite) => sprite.value));
        }
    }
